...
document.addEventListener("viewTransitionOnNewDocument", (event) => {
  ...
  // The `ViewTransition` object associated with this transition.
  const transition = event.transition;
  
  // Retrieve the list of names captured on the old Document.
  const oldNames = event.oldNames;
  
  // Retrieve the context generated by the old Document.
  ...
  // The browser defers painting the new Document until all render-blocked
  // resources have been fetched or timed out.
  // The `domUpdated` promise can be used to inspect when the above is
  // done and add viewTransitionNames based on what the new Document's
  // first paint would look like.
