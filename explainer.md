...
*   Root Transitions : The full page content animates between two web pages with
    optional static UI elements on top.
    [Shared axis](https://material.io/design/motion/the-motion-system.html#shared-axis)
    shows these examples. #1 and #2 have a transition with static elements while #3
    doesn't.
...
*   The Live Animatable Properties could reference resources which are
    ...
    Note that painting cross-origin resources in the image generated by element()
    is not a security concern since the pixel content will never be accessible to
    the incoming page.
